...
// Parallel to that, we process an ASCII input stream inverting the 
// case of each character before sending it out.

// In order to keep external components at a minimum, we use the 
// builtin LED and a single button.  The blinking is implemented 
// by using the "Timer/Counter0 Compare Match A" interrupt.
...
byte mode = 0;                   // the mode of the blinker
...
void setup() {
  ...
  digitalWrite(LEDPIN,1);        // switch LED on
  pinMode(BUTTON, INPUT_PULLUP); // initialize button
  pinMode(BUTTON_GND, OUTPUT);   // provide GND for the button
