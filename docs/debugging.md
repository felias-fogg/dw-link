...
If, instead, you want to use dw-link in the Arduino IDE 2, read on. Using dw-link in the Arduino IDE 2 is straightforward. Before starting, you must only [install three new board manager URLs and the respective board packages](installation.md#setting-up-the-debugging-software). And then you can use the debugging feature of the Arduino IDE 2. 
...
![ide2-4-new](pics/ide2-4-new.png)The debugging panes are orgnized as folloes. Pane A contains the debug controls. From left to right:
...
Global variables are, by default, not displayed. However, you can set a watch expression in the Watch pane to display a global variable's value. Or you can disable link time optimization (LTO) in the `Tools` menu. Then all global variables show up, except for one (which seems to be an error of the IDE).
